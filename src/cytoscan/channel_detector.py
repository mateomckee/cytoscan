...
from cytoscan.config import DetectionConfig, ExperimentConfig
...
def detect_walls(d_cfg: DetectionConfig, exp_cfg: ExperimentConfig, br_frame: str):
    ...
    expected_w_px = d_cfg.expected_channel_width_um / exp_cfg.pixel_size_um
    axis = w / 2.0

    profile_pos = grad_pos.sum(axis=0)
    tol_px = d_cfg.channel_width_search_tolerance * expected_w_px
    ...
    hw = d_cfg.wall_strip_half_width
    deg = d_cfg.channel_wall_degree
    right_centers, right_coeffs, right_hw_arr = _fit_wall_at(gray, grad_pos, right_anchor, hw, deg)

    narrow_tol_px = d_cfg.wall_parallelism_search_fraction * measured_w_px
    ...
    suggested_inset = int(np.percentile(np.concatenate([left_hw_arr, right_hw_arr]), 95)) + d_cfg.channel_wall_base_inset
...
def detect_interface(d_cfg: DetectionConfig, br_frame: str, left_coeffs: np.ndarray, right_coeffs: np.ndarray, inset: int):
    ...
    sigma = d_cfg.interface_ridge_sigma_px
    ridge = np.abs(gaussian_filter1d(gray.astype(np.float32), sigma=sigma, order=2, axis=1))

    max_jump = int(d_cfg.interface_dp_max_jump_px)
    lam = float(d_cfg.interface_dp_jump_penalty)
    INF = np.float32(1e9)
